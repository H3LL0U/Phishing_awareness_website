...
def setup():
    ...
    ca = certifi.where()
    connection_db = pymongo.MongoClient(
        config["MONGO_DB_LINK"],
        maxPoolSize=None,
        tlsCAFile=ca)
...
def link_redirect(redirect_type, encrypted_email):
    ...
    if email_id is None:
        return home()
    properties = get_email_properties(connection_db, encrypted_email)
    ...
    add_property_to_documents(
        ...
        filter_query={
            "_id": email_id})
    if properties.get("user_state") < 1:
        add_property_to_documents(
            connection_db,
            "user_state",
            1,
    ...
    redirect_type_doc = get_documents_by_query(
        connection_db, {
            "allowed_type": redirect_type}, collection_name="redirect_types")
    ...
    try:
        date_of_visit_value = get_email_properties(
            connection_db,
            decrypt_value(encrypted_email),
            auto_decrypt=False)["date_of_visit"]
    except KeyError:
        date_of_visit_value = "None"

    if date_of_visit_value == "None":
        new_date_of_visit_value = {
            redirect_type: datetime.datetime.fromtimestamp(
                time.time()).strftime('%Y-%m-%d %H:%M:%S')}
    elif redirect_type in date_of_visit_value:
        new_date_of_visit_value = date_of_visit_value
    else:
        new_date_of_visit_value = date_of_visit_value
        new_date_of_visit_value[redirect_type] = datetime.datetime.fromtimestamp(
            time.time()).strftime('%Y-%m-%d %H:%M:%S')
    add_property_to_documents(
        connection_db,
        "date_of_visit",
    ...
    response = make_response(
        render_template(
            "login.html",
            root_name="/",
            username=decrypt_value(encrypted_email)))
...
def typed():
    ...
    properties = get_email_properties(connection_db, encrypted_email)
    if encrypted_email is None and properties:
        return home(warn=True)

    email_id = properties.get("_id")
    # update the database if user started typing in the password field
    if email_id:
        add_property_to_documents(
            connection_db,
            "started_typing",
            ...
            filter_query={
                "_id": email_id})
        if properties.get("user_state") < 3:
            add_property_to_documents(
                connection_db, "user_state", 3, filter_query={
                    "_id": email_id})
    return home(warn=True)
...
def api_visit_data():
    ...
    '''
    try:
        visited_users = get_documents_by_query(
            connection_db, {"date_of_visit": {"$ne": "None"}})
        visited_users = structure_date_data(
            [visited_user["date_of_visit"] for visited_user in visited_users])

        response = dict()
        response["visited"] = len(
        ...
        response["typed"] = len(
            get_documents_by_query(
                connection_db, {
                    "user_state": 3}))
        response["typed_email"] = len(
            get_documents_by_query(
                connection_db, {
                    "user_state": 2}))
        response["not_visited"] = len(
            get_documents_by_query(
                connection_db, {
                    "user_state": 0}))
...
def api_email_data():
    ...
    try:
        sent_emails = get_documents_by_query(
            connection_db, {"date_of_email": {"$ne": "None"}})
        sent_emails = structure_date_data(
            [sent_email["date_of_email"] for sent_email in sent_emails])
...
def remove_user():
    try:
        ...
        if delte_result and delte_result.deleted_count == 1:
            ...
            return jsonify({"Failure": "No user removed"}), 200
...
def validate_user_with_cookie():
    try:
        ...
        properties = get_email_properties(connection_db, user)
        user_id = properties.get("_id")
        check_status = user == request.cookies.get("login")
        update_db = "update_db" in request.json and request.json.get(
            "update_db") is True
        if update_db and check_status:
            add_property_to_documents(
                ...
                filter_query={
                    "_id": user_id})
            if properties.get("user_state") < 2:
                add_property_to_documents(
                    connection_db, "user_state", 2, filter_query={
                        "_id": user_id})
